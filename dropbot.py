...
def send_telegram_message(message):
    ...
    try:
        ...
        print(
            f"{Fore.RED+Style.BRIGHT}Failed to send Telegram notification: {e}{Style.RESET_ALL}"
        )
...
█▄█ ██▄ █░▀█ ██▄ █▀▄ █▄█ █▄█ ▄█
    ...
    print(
        Fore.CYAN
        + Style.BRIGHT
        + f"Up time bot: {int(days)} hari, {int(hours)} jam, {int(minutes)} menit, {int(seconds)} detik\n\n"
    )
...
class DropstabClient:
    ...
    def login(self):
        json_data = {
            "webAppData": self.query_data
        }  # Renamed to avoid confusion with the json module
        try:
            response = requests.post(
                f"{self.API_URL}/api/auth/login", json=json_data
            )  # Use json parameter
            response.raise_for_status()
            data = response.json()
            if (
                "jwt" in data
                and "access" in data["jwt"]
                and "token" in data["jwt"]["access"]
            ):
                self.token = data["jwt"]["access"]["token"]
                return True
            else:
                print(
                    f"{Fore.RED+Style.BRIGHT}{self.account['username']} | Login failed: JWT structure not as expected{Style.RESET_ALL}"
                )
                return False
        except requests.RequestException as e:
            print(
                f"{Fore.RED+Style.BRIGHT}{self.account['username']} | Login failed due to request error{Style.RESET_ALL}"
            )
            return False
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                try:
                    ...
                    print(
                        f"{Fore.RED+Style.BRIGHT}         Request failed with 400 and no JSON response{Style.RESET_ALL}"
                    )
            else:
    ...
    def get_user_info(self):
        try:
            ...
            print(
                f"{Fore.RED+Style.BRIGHT}{self.account['username']} | Failed to get user info{Style.RESET_ALL}"
            )
            return None
    ...
    def ghalibie(self):
        ...
        try:
            return self._make_authenticated_request(
                "PUT", "/api/user/applyRefLink", json_data
            )
        except requests.RequestException as e:
            print(f"{Fore.RED+Style.BRIGHT}Request failed: {e}{Style.RESET_ALL}")
            return None
    ...
    def daily_bonus(self):
        try:
            ...
            print(
                f"{Fore.RED+Style.BRIGHT}[ Daily ]: Failed to claim daily bonus{Style.RESET_ALL}"
            )
            return None
    ...
    def welcome_bonus(self):
        try:
            ...
            print(
                f"{Fore.RED+Style.BRIGHT}[ Welcome Bonus ]: Failed to claim daily bonus{Style.RESET_ALL}"
            )
            return None
    ...
    def ether_drops_subscription(self):
        try:
            return self._make_authenticated_request(
                "GET", "/api/etherDropsSubscription"
            )
        except requests.RequestException:
            print(
                f"{Fore.RED+Style.BRIGHT}[ Balance]: Failed to get ether drops subscription{Style.RESET_ALL}"
            )
            return None
    ...
    def ref_info(self):
        try:
            ...
            print(
                f"{Fore.RED+Style.BRIGHT}[ Refferal ]: Failed to get ref info{Style.RESET_ALL}"
            )
            return None
    ...
    def claim_ref(self):
        try:
            ...
            if result.get("status") == "OK":
                print(
                    f"{Fore.GREEN+Style.BRIGHT}[ Claim Reff ]: Claim Successfully{Style.RESET_ALL}"
                )
            elif result.get("message") == "Already claimed":
                print(
                    f"{Fore.YELLOW+Style.BRIGHT}[ Claim Reff ]: Already Claimed{Style.RESET_ALL}"
                )
            return result
        except requests.RequestException:
            return None
    ...
    def active_task_list(self):
        try:
            ...
            print(
                f"{Fore.RED+Style.BRIGHT}[ Task ]: Failed to get active task list{Style.RESET_ALL}"
            )
            return None
    ...
    def verify_task(self, task_id):
        try:
            result = self._make_authenticated_request(
                "PUT", f"/api/quest/{task_id}/verify"
            )
            return result.get("status", "FAILED")
        except requests.RequestException:
            print(
                f"{Fore.RED+Style.BRIGHT}Error verifying task {task_id}{Style.RESET_ALL}"
            )
            return "FAILED"
    ...
    def claim_task(self, task_id):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(
                    f"{Fore.RED+Style.BRIGHT}          -> Request failed {e}{Style.RESET_ALL}"
                )
                if attempt < max_retries - 1:
                    print(
                        f"{Fore.YELLOW+Style.BRIGHT}          -> Retrying in {retry_delay} seconds...{Style.RESET_ALL}"
                    )
                    time.sleep(retry_delay)
                else:
                    print(
                        f"{Fore.YELLOW+Style.BRIGHT}          -> Failed to claim. May be Claimable in Next Loop.{Style.RESET_ALL}"
                    )
                    return "FAILED"
    ...
    def auto_farming(self):
        ...
        if user_info:
            ...
            print(
                f"{Fore.GREEN+Style.BRIGHT}========[ {Fore.WHITE}{self.account['username']}{Style.RESET_ALL}{Fore.GREEN+Style.BRIGHT} ]======== {Style.RESET_ALL}"
            )
        else:
            print(
                f"{Fore.RED+Style.BRIGHT}=======[ User Info Not Found ]========={Style.RESET_ALL}"
            )
            return points
        ...
        if ghalibie is not None:
            if "id" in ghalibie and "tgId" in ghalibie:
                print(
                    f"{Fore.GREEN+Style.BRIGHT}[ Refferal ]: Applied Successfully{Style.RESET_ALL}"
                )
            elif ghalibie.get("code") == "USER_ALREADY_APPLY_REF_LINK_ERROR":
                print(
                    f"{Fore.RED+Style.BRIGHT}[ Refferal ]: Already Applied Before (NOT FRESH){Style.RESET_ALL}"
                )
        else:
            print(
                f"{Fore.RED+Style.BRIGHT}[ Refferal ]: Failed to apply referral{Style.RESET_ALL}"
            )
        ...
        if welcome is not None:
            if welcome["result"] == False:
                print(
                    f"{Fore.YELLOW+Style.BRIGHT}[ Welcome Bonus ]: Already Claimed{Style.RESET_ALL}"
                )
            else:
                print(
                    f"{Fore.GREEN+Style.BRIGHT}[ Welcome Bonus ]: Successfully Claimed. Bonus: {welcome['bonus']}{Style.RESET_ALL}"
                )
        if daily_bonus and daily_bonus.get("result"):
            print(
                f"{Fore.GREEN+Style.BRIGHT}[ Daily ]: Successfully Claimed Daily Bonus | Bonus: {daily_bonus['bonus']} | Streak: {daily_bonus['streaks']}{Style.RESET_ALL}"
            )

        subscription = self.ether_drops_subscription()
        if subscription:
            print(
                f"{Fore.GREEN+Style.BRIGHT}[ Balance ]: {subscription['balance']} | Course: {subscription['course']} | Available: {subscription['available']} | Claimed: {len(subscription['claimed'])}{Style.RESET_ALL}"
            )
    ...
    def auto_claim_ref(self):
        ...
        if ref_info:
            print(
                f"{Fore.GREEN+Style.BRIGHT}[ Refferal ]: Ref Code: {ref_info.get('code')} | Total Ref: {ref_info['referrals'].get('total', 0)} | Total Reward: {ref_info.get('totalReward', 0)} | Available Claim: {ref_info.get('availableToClaim', 0)}{Style.RESET_ALL}"
            )
            self.claim_ref()
    ...
    def process_tasks(self):
        ...
        if active_tasks:
            ...
            if not has_non_invite_tasks:
                print(
                    f"{Fore.YELLOW+Style.BRIGHT}[ Tasks ]: Only invite tasks found. Skipping.{Style.RESET_ALL}"
                )
                return
                ...
                if "quests" in task:
                    for quest in task["quests"]:
                        ...
                        if "Invite" in quest_name:
                            print(
                                f"{Fore.YELLOW+Style.BRIGHT}          -> {quest_name} Skipped         {Style.RESET_ALL}"
                            )
                            continue
                        if quest_status == "COMPLETED":
                            print(
                                f"{Fore.GREEN+Style.BRIGHT}          -> {quest_name} Completed         {Style.RESET_ALL}"
                            )
                            continue
                        elif quest_status == "VERIFICATION":
                            print(
                                f"{Fore.CYAN+Style.BRIGHT}          -> {quest_name} Verification        {Style.RESET_ALL}"
                            )
                            continue
                        else:
                            print(
                                f"{Fore.YELLOW+Style.BRIGHT}          -> {quest_name} Starting            {Style.RESET_ALL}",
                                end="\r",
                                flush=True,
                            )
                        ...
                        if verify_result == "OK":
                            print(
                                f"{Fore.YELLOW+Style.BRIGHT}          -> {quest_name} Started            {Style.RESET_ALL}"
                            )
                            ...
                            if claim_result == "OK":
                                print(
                                    f"{Fore.GREEN+Style.BRIGHT}          -> {quest_name} Claimed       {Style.RESET_ALL}"
                                )
                            elif claim_result == "QUEST_NOT_COMPLETED":
                                print(
                                    f"{Fore.CYAN+Style.BRIGHT}          -> {quest_name} Quest in Verification {Style.RESET_ALL}"
                                )
                            else:
                                print(
                                    f"{Fore.YELLOW+Style.BRIGHT}          -> {quest_name} Failed to Claim. May be Claimable in Next Loop.{Style.RESET_ALL}"
                                )
                        else:
                            print(
                                f"{Fore.RED+Style.BRIGHT}          -> {quest_name} Failed to Start        {Style.RESET_ALL}"
                            )
        else:
            print(
                f"{Fore.YELLOW+Style.BRIGHT}[ Tasks ]: No active tasks found.{Style.RESET_ALL}"
            )
...
def process_accounts(query_file):
    ...
    for query in queries:
        ...
        total_points += points
        print(
            f"{Fore.GREEN+Style.BRIGHT}Account {account_count} Finished. Points: {points}{Style.RESET_ALL}"
        )
...
def main():
    ...
    while True:  # Infinite loop to keep the process running
        ...
        print(
            f"{Fore.GREEN+Style.BRIGHT}Total Accounts: {account_count}{Style.RESET_ALL}"
        )
        ...
        for remaining_time in range(1800, 0, -1):
            ...
            print(
                f"{random.choice([Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE])+Style.BRIGHT}==== [ All accounts processed, Next loop in {minutes:02d}:{seconds:02d} ] ===={Style.RESET_ALL}",
                end="\r",
                flush=True,
            )
            time.sleep(1)
